        ...
        <main className="flex flex-col items-center h-full w-full gap-5 bg-[url(https://wallpapers.com/images/hd/minimalist-abstract-bq91czhp5gxmprez.jpg)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5 mb-50 mt-20 pr-15 pl-15">
                {jsonData.filter((item) => item.First_name.toLowerCase().includes(nameSearch.toLowerCase())).map((item) => (
                    ...
                    :
                    <div className="hover:scale-105 border-3 border-green-700 rounded-xl bg-white cursor-pointer p-3 flex flex-col justify-between" key={item.id}>
                        <div>
                            <div className="flex flex-row justify-between">
                                <p className="font-bold">{item.Last_name[0]}. {item.First_name}</p>
                                <p className="text-green-700 border-1 border-green-700 rounded-xl p-2">{item.type}</p>
                            </div>
                            <img src="https://tse2.mm.bing.net/th/id/OIP.dcyZasadquPyCfekD9g7jAHaFW?pid=Api&h=220&P=0" alt="" className="h-[35%] w-[35%]"></img>
                            <p>Age: {item.age}</p>
                            {(item.point > 59) ? <p className="text-green-500 font-bold">Grade: {item.point}</p> : <p className="text-red-500 font-bold"> Grade: {item.point}</p>}
                            <p>Gender: {item.gender}</p>
                        </div>
                ...
                <div className="col-span-5 flex justify-center">
                    <button onClick={() => router.push("/")} className="text-white text-sm">click the logo to go to the homepage</button>
                </div>
            </div>
        </main>
    );
